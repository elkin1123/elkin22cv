...
class ExperienciaLaboral(models.Model):
    idperfilconqueestaativo = models.ForeignKey(DatosPersonales, on_delete=models.CASCADE, related_name='experiencias')
    ...
    telefoncontactoempresarial = models.CharField(max_length=60, blank=True, null=True)
    fechainicogestion = models.DateField()
    fechafingestion = models.DateField(blank=True, null=True)
    descripcionfunciones = models.CharField(max_length=100)
    activarparaqueeseveaenfront = models.BooleanField(default=True)
...
class Reconocimiento(models.Model):
    idperfilconqueestaativo = models.ForeignKey(DatosPersonales, on_delete=models.CASCADE, related_name='reconocimientos')
...
class CursoRealizado(models.Model):
    idperfilconqueestaativo = models.ForeignKey(DatosPersonales, on_delete=models.CASCADE, related_name='cursos')
...
class VentaGarage(models.Model):
    idperfilconqueestaatcivo = models.ForeignKey(DatosPersonales, on_delete=models.CASCADE, related_name='ventas_garage')
    nombrepreducto = models.CharField(max_length=100)
    ...
    activparaqueuseveaefront = models.BooleanField(default=True)

    def __str__(self):
        return self.nombrepreducto